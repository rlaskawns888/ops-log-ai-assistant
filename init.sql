...
-- DROP TABLES
-- =========================================================
-- 기존 DB를 완전히 초기화하고 다시 만들 때만 사용
-- 데이터가 전부 삭제되니 주의

-- DROP TABLE IF EXISTS ai_feedbacks CASCADE;
-- DROP TABLE IF EXISTS log_analysis_references CASCADE;
-- DROP TABLE IF EXISTS log_analysis_results CASCADE;
-- DROP TABLE IF EXISTS log_analysis_requests CASCADE;
...
CREATE INDEX idx_log_analysis_requests_service_name
ON log_analysis_requests(service_name);

CREATE INDEX idx_log_analysis_requests_environment
ON log_analysis_requests(environment);

CREATE INDEX idx_log_analysis_requests_log_level
ON log_analysis_requests(log_level);

CREATE INDEX idx_log_analysis_results_request_id
ON log_analysis_results(request_id);

CREATE INDEX idx_log_analysis_results_severity
ON log_analysis_results(severity);

CREATE INDEX idx_log_analysis_references_result_id
ON log_analysis_references(result_id);

CREATE INDEX idx_log_analysis_references_document_chunk_id
ON log_analysis_references(document_chunk_id);
...
COMMENT ON COLUMN log_analysis_results.total_tokens IS 'LLM 요청/응답 전체 토큰 수';
COMMENT ON COLUMN log_analysis_results.latency_ms IS 'AI 분석 처리에 걸린 시간. 밀리초 단위';
